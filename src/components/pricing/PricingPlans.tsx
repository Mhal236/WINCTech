    ...
    <div className="py-8 px-4 md:px-0">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8 md:mb-12">
          <h2 className="text-2xl md:text-3xl font-bold text-[#3d99be] mb-4">Simple, Transparent Pricing</h2>
            ...
            <div key={plan.name} className="relative">
              {plan.popular && (
                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 z-20">
                  <span className="bg-[#3d99be] text-white px-4 py-1 rounded-full text-sm font-medium shadow-sm">
                    Most Popular
                  </span>
                </div>
              )}
              <Card 
                className={`rounded-lg overflow-hidden transition-all duration-300 hover:shadow-xl flex flex-col h-full ${
                  plan.popular ? 'border-[#3d99be] ring-1 ring-[#3d99be] transform md:-translate-y-2 mt-3' : ''
                }`}
              >
                <CardHeader className={`p-4 md:p-6 ${
                  plan.popular 
                    ? 'bg-gradient-to-b from-[#3d99be]/30 via-[#3d99be]/20 to-white/90' 
                  ...
                  <ul className="space-y-2 md:space-y-3">
                    {plan.features.map((feature) => (
                      <li key={feature} className="flex items-center gap-2 text-sm md:text-base">
                        <Check className="h-4 w-4 md:h-5 md:w-5 text-[#3d99be] flex-shrink-0" />
